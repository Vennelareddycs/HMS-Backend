...
public ResponseEntity<?> createRoom(@Valid @RequestBody Room room){
    if (roomRepository.existsByroomNo(room.getRoomNo())) {
        return ResponseEntity
                .badRequest()
                .body(new MessageResponse(" room is already taken!"));
    }
    ...
    @GetMapping("room/{id}")
    public ResponseEntity<Room> getroomById(@PathVariable(value="id") int roomId)throws ResourceNotFoundException {
        Room room=roomRepository.findById(roomId)
                .orElseThrow(()->new ResourceNotFoundException("room  not found:: "+roomId));
        return ResponseEntity.ok().body(room);
    }
    @PutMapping("room/edit/{id}")
    public ResponseEntity<Room> updatePayment(@PathVariable(value = "id") int  roomId,
    ...
    public Map<String,Boolean> deleteroom(@PathVariable(value = "id") int  roomId)throws ResourceNotFoundException {
        Room room=roomRepository.findById(roomId)
                .orElseThrow(() -> new ResourceNotFoundException("room not found for this id :: " +roomId ));
