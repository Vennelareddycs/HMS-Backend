...
public class IssueBillController {
    ...
    @GetMapping("bill/{id}")
    public ResponseEntity<IssueBills> getBillById(@PathVariable(value="id") int billId)throws ResourceNotFoundException {
    ...
    public ResponseEntity<IssueBills> updateBill(@PathVariable(value = "id") int  billId,
    ...
    public Map<String,Boolean> deleteBill(@PathVariable(value = "id") int  billId)throws ResourceNotFoundException {
        IssueBills issueBills=issueBillRepository.findById(billId)
                .orElseThrow(() -> new ResourceNotFoundException("bill not found for this id :: " +billId ));
