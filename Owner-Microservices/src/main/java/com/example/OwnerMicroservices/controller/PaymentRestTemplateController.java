...
public class PaymentRestTemplateController {
    ...
    static  final String PAYMENT_URL="http://localhost:8084/Receptionist/";

    @GetMapping("/restpayment/{id}")
    public String fetchPayment(@PathVariable(value = "id") int id){
        return restTemplate.exchange(PAYMENT_URL+"payment/"+id, HttpMethod.GET,null,String.class).getBody();

    }
    @GetMapping("/restlistpayment")
    public String fetchallpayment(){
        return restTemplate.exchange(PAYMENT_URL+"payment/list",HttpMethod.GET,null,String.class).getBody();
    }

    @PostMapping("/restpaymentave")
    public String addPayment( @Valid @RequestBody Payment payment){
        return restTemplate.postForObject(PAYMENT_URL+"payment/save",payment,String.class);
    }
...
//                PAYMENT_URL+"payment/edit/"+id, HttpMethod.PUT, entity, String.class).getBody();
//    }
//
...
//
//        return restTemplate.exchange(
//                PAYMENT_URL+"payment/delete/"+id, HttpMethod.DELETE, null, String.class).getBody();
//    }
