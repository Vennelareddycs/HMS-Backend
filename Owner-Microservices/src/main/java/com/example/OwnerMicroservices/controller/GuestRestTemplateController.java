...
public class GuestRestTemplateController {
    ...
    static final String GUEST_URL = "http://localhost:8089/Receptionist/";

    @GetMapping("restguest/{id}")
    public String fetchguest(@PathVariable(value = "id") int id) {
        return restTemplate.exchange(GUEST_URL + "guest/" + id, HttpMethod.GET, null, String.class).getBody();

    }

    @GetMapping("/restlistguest")
    public String fetchallguest(){
        return restTemplate.exchange(GUEST_URL+"guest/list",HttpMethod.GET,null,String.class).getBody();
    }
    @PostMapping("/restguestsave")
    public String addPayment(@Valid @RequestBody Guest guest){
        return restTemplate.postForObject(GUEST_URL+"guest/save",guest,String.class);
    }
    ...
    public String updateguest(@PathVariable(value = "id") int id, @RequestBody Guest guest ) {
        ...
        return restTemplate.exchange(
                GUEST_URL+"guest/edit/"+id, HttpMethod.PUT, entity, String.class).getBody();
    }
    @DeleteMapping("/deleteguest/{id}")
    public String deleteRoom(@PathVariable(value = "id") int id, @RequestBody Guest Guest ) {

        return restTemplate.exchange(
                GUEST_URL+"guest/delete/"+id, HttpMethod.DELETE, null, String.class).getBody();
    }
}
