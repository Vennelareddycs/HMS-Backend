...
public class EmployeeRestTemplateController {
    ...
    static  final String EMPLOYEE_URL="http://localhost:8082/Manager/";

    @GetMapping("/restemployee/{id}")
    public String fetchroom(@PathVariable(value = "id") int id){
        return restTemplate.exchange(EMPLOYEE_URL+"employee/"+id, HttpMethod.GET,null,String.class).getBody();

    }
    @GetMapping("/rest/listemployees")
    public String fetchallEmployees(){
        return restTemplate.exchange(EMPLOYEE_URL+"employee/list",HttpMethod.GET,null,String.class).getBody();
    }
    @PostMapping("rest/employeesave")
    public String addEmployee(@Valid @RequestBody Employee employee){
        return restTemplate.postForObject(EMPLOYEE_URL+"employee/save",employee,String.class);
    }
    ...
    public String updateEmpoyee(@PathVariable(value = "id") int id, @RequestBody Employee employee ) {
        ...
        return restTemplate.exchange(
                EMPLOYEE_URL+"employee/edit/"+id, HttpMethod.PUT, entity, String.class).getBody();
    }
    ...
    public String deleteEmployee(@PathVariable(value = "id") int id, @RequestBody Employee employee ) {
        ...
        return restTemplate.exchange(
                EMPLOYEE_URL+"employee/delete/"+id, HttpMethod.DELETE, null, String.class).getBody();
    }
}
