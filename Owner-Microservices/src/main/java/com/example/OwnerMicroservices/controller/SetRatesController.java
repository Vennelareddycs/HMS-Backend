...
public class SetRatesController {
    ...
    public ResponseEntity<SetRates> getrateById(@PathVariable(value="id") int rateId)throws ResourceNotFoundException {
    ...
    public ResponseEntity<SetRates> updaterate(@PathVariable(value = "id") int  rateId,
    ...
    public Map<String,Boolean> deleterate(@PathVariable(value = "id") int  rateId)throws ResourceNotFoundException {
        SetRates rate=setRatesRepository.findById(rateId)
                .orElseThrow(() -> new ResourceNotFoundException("rate not found for this id :: " +rateId ));
